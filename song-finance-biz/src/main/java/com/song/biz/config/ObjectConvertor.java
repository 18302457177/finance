...
public interface ObjectConvertor {
    ...
    @Mappings({
            @Mapping(target = "valueAddedTaxCate", expression = "java(com.song.biz.enums.ValueAddedTaxCateEnum.getMessage(source.getValueAddedTaxCate()))"),
            @Mapping(target = "accountingStandard", expression = "java(com.song.biz.enums.AccountingStandardEnum.getMessage(source.getAccountingStandard()))")
    })
    ...
    Folder toFolder(CreateFolderForm source);

    GetFolderVo toGetFolderVo(Folder source);

    List<ListFolderVo> toListFolderVo(List<Folder> source);

    ListFileVo toListFileVo(File source);
}